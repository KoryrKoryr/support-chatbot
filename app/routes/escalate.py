...
def escalate_request(data: EscalationRequest):
    ...
    email_logger.info(f"🚨 Escalate API called by {data.email} — question='{data.question[:50]}...'")

    try:
        send_escalation_email(data.question, data.name, data.email)
        email_logger.info(f"📧 Escalation email sent to support for {data.email}")
        return {"message": "Escalation email sent successfully"}
    except Exception as e:
        email_logger.error(f"❌ Escalation email failed for {data.email}: {e}")
